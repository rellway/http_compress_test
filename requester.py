...
def CreateRequesters(num_senders, max_qps, url, logger_obj, 
    ...
    for i in range(num_senders):
        generator_obj = generator.BidRequestGenerator()
...
class Requester(threading.Thread):
    ...
    def Start(self):
        ...
        while self._ShouldSendMoreRequests():
            ...
            try:
                status, data = self._sender(request)
            except:
                status = 404
                data = ""
            finally:
    ...
    def _GenerateRequest(self):
        bid_request = self._generator.GenerateRequest()
...
def main():
    ...
    requesters = CreateRequesters(opts.num_threads, opts.max_qps, opts.url,
                                logger_obj, opts.seconds, opts.requests )
